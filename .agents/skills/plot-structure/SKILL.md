...
5. Build the arc through conversation using the **Web Novel 10-Chapter Pacing Formula (CRITICAL)**:
   - Chapters 1-3: Establish an oppressive problem/crisis/enemy.
   - Chapters 4-6: The protagonist formulates a plan with absolute confidence.
   - Chapters 7-8: A twist or complication occurs.
   - Chapter 9: The Climax—absolute dimensional-strike destruction.
   - Chapter 10: Hook for the next arc.
...
During chapter writing, flag any `planted` items that haven't been paid off as reminders. **CRITICAL**: Before drafting any new volume/arc outline, the AI MUST explicitly review this 'Foreshadowing Bank' to locate unresolved setups, and integrate their payoffs into the new arc to avoid unresolved plot threads (挖坑必填).
