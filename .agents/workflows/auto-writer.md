...
2. **第二步：构筑无菌防崩坏环境 (Context Sandbox)**
   ...
   - 从 `characters/` 目录调取第 `N` 章对应必须出场的核心人物卡片（务必包含 Voice Bank 专属语料库和 Dynamic Status 动态状态栏）。

3. **第三步：生成第N章・三段式硬核细纲 (Beat-by-Beat Outline)**
   - 基于第二步的隔离提示词，以及《黄金10章节拍器》公式，严格写出第 N 章的"起、承、转、合"细纲。
   - 使用 `chapter-template.md` 中的三段式分镜格式。
   - （等待用户确认，或直接进入第四步）

4. **第四步：分块拼装与审查生成 (Chunking & Linter)**
   - 严禁一次性输出全文。**必须分三次生成**：
     * **Chunk 1**：生成【起】（约800字）。
     * **Chunk 2**：生成【承/转】（约800字）。
     * **Chunk 3**：生成【合/高潮】（约800字）。
   - 将三次内容拼接为完整的 2000-2500 字的正文，通过 UTF-8 编码保存至 `chapters/chapter-{N}.md`。
   - **执行硬性代码 Linter**：强制静默执行系统命令行 `python scripts/linter.py chapters/chapter-{N}.md`。
   - **进入反AI质检节点**：检查是否贯彻了长短句结合公式？是否在高潮段落精准执行了"五感替代法降速描写"？
